...
class Login_Window:
    def __init__(self, root):
        ...
        original_image = Image.open(r"C:\Users\Abhishek Jaiswal\OneDrive\Desktop\FRAS Project\Images\login1.jpg")
...
#icon img
        ...
        back_button = Button(self.root, text="Back", cursor="hand2",font=("times new roman", 10, "bold"),width=18, bg="white", command=self.go_back,activebackground="white")
        back_button.place(x=1120, y=50)  # Adjust x and y coordinates as needed
...
#     # """"""""""""work on login button""""""""""""
    def login(self,event=None):
        if self.txtuser.get() == "" or self.txtpass.get() == "":
            messagebox.showerror("Error", "Please Enter Email and Password to Login",parent=self.root)
        else:
            conn = mysql.connector.connect(host="localhost", username="root", password="Hi@itsabhish1", database="face_recognition")
            my_cursor = conn.cursor()
        ...
        row = my_cursor.fetchone()
        if row:
            response=messagebox.askquestion("Welcome", "Do you want to Login",parent=self.root)
            if response=='yes':           
...
class Register:
    def __init__(self, root):
       ...
       #register label
        ...
        back_button = Button(self.root, text="Back",cursor="hand2", font=("times new roman", 10, "bold"),width=18, bg="white", command=self.go_back,activebackground="white")
        back_button.place(x=1070, y=75)  # Adjust x and y coordinates as needed
        #   # Bind the backspace key key to the go_back method
        # self.root.bind('<BackSpace>', lambda event: self.go_back())
    ...
    def register_data(self):
        ...
        if not self.is_strong_password(self.var_pass.get()):
            ...
            messagebox.showwarning("Warning", "Email must contain @gmail.com",parent=self.root)
        else:
            conn=mysql.connector.connect(host="localhost", username="root", password="Hi@itsabhish1", database="face_recognition")
            my_cursor=conn.cursor()
            query=("select * from register where email=%s")
...
#Student BUTTONS
        ...
        #ChatBot Button
        img_chat=Image.open(r"C:\Users\Abhishek Jaiswal\OneDrive\Desktop\FRAS Project\Images\chatbot.png")
        img_chat=img_chat.resize((100,100),Image.Resampling.LANCZOS)
        ...
        bchat.place(x=1000,y=100,width=110,height=110)

        b1_chat=Button(bg_img,text="ChatBot",cursor="hand2",command=self.chatbot,font=("times new roman",12,"bold"),bg="green",fg="white",activebackground="green",activeforeground="white")
        b1_chat.place(x=1000,y=200,width=110,height=40)

        back_button = Button(self.root, text="Back", cursor="hand2",font=("times new roman", 10, "bold"),width=18, bg="yellow", command=self.go_back,activebackground="white")
        back_button.place(x=1100, y=20)  # Adjust x and y coordinates as needed
